...
$$\pi^\ast = \arg\max_\pi \mathbb{E}_{\pi} [r(s, a)] - \beta \, \mathbb{D}_{\text{KL}}(\pi \parallel \pi_{\text{SFT}})$$
...
$$\pi^\ast = \arg\max_\pi \mathbb{E} \left[ \sum_{t=1}^T \gamma^t \, r_t \right] = J(\pi)$$
...
- The **optimal policy $\pi^\ast$** achieves the maximum expected return from every state.
...
From prior theoretical work, the mathematical optimization problem above has a known exact optimal policy solution ($\pi^\ast$) expressed as a function of the ground-truth reward:
...
By taking the log of the optimal policy equation and isolating the reward term, we can express the implicit reward function $r(x, y)$ purely in terms of the optimal policy ($\pi^\ast$), the reference policy ($\pi_{\text{ref}}$), and the partition function ($Z(x)$):
...
$$r(x, y^w) - r(x, y^l) = \left( \beta \log \frac{\pi^\ast(y^w \mid x)}{\pi_{\text{ref}}(y^w \mid x)} + \beta \log Z(x) \right) - \left( \beta \log \frac{\pi^\ast(y^l \mid x)}{\pi_{\text{ref}}(y^l \mid x)} + \beta \log Z(x) \right)$$

$$r(x, y^w) - r(x, y^l) = \beta \log \frac{\pi^\ast(y^w \mid x)}{\pi_{\text{ref}}(y^w \mid x)} - \beta \log \frac{\pi^\ast(y^l \mid x)}{\pi_{\text{ref}}(y^l \mid x)}$$
